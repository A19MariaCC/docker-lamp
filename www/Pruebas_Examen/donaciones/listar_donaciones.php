...
if(isset($_GET["id"])){
    $idDonante = $_GET["id"];
    $stmt = $conPDO->prepare("SELECT donantes.nombre, donantes.apellidos, donantes.edad, donantes.grupoSanguineo, donantes.codPostal, donantes.telefonoMovil, historico.fechaDonacion, historico.proximaDonacion FROM donantes, historico
    WHERE donantes.id = historico.idDonante AND donantes.id = :id ORDER BY historico.fechaDonacion DESC");
